# Extensions

The `ModuleRegistry` provides a hook-based extension system for adding modular capabilities to the runtime.

## Module structure

Each module has an info block, a context pointer, and optional lifecycle hooks:

```zig
const MyModule = struct {
    data: u32 = 0,

    fn start(context: *anyopaque, runtime: zero_native.extensions.RuntimeContext) anyerror!void {
        _ = runtime;
        const self: *@This() = @ptrCast(@alignCast(context));
        self.data = 42;
    }

    fn command(context: *anyopaque, runtime: zero_native.extensions.RuntimeContext, cmd: zero_native.extensions.Command) anyerror!void {
        _ = runtime;
        const self: *@This() = @ptrCast(@alignCast(context));
        if (std.mem.eql(u8, cmd.name, "reset")) self.data = 0;
    }
};
```

## Registering modules

```zig
var my_module = MyModule{};
const caps = [_]zero_native.extensions.Capability{.{ .kind = .native_module }};
const modules = [_]zero_native.extensions.Module{.{
    .info = .{ .id = 1, .name = "my-module", .capabilities = &caps },
    .context = &my_module,
    .hooks = .{ .start_fn = MyModule.start, .command_fn = MyModule.command },
}};
const registry = zero_native.extensions.ModuleRegistry{ .modules = &modules };

var runtime = zero_native.Runtime.init(.{
    .platform = my_platform,
    .extensions = registry,
});
```

## Module fields

<table>
  <thead>
    <tr>
      <th>Field</th>
      <th>Type</th>
      <th>Description</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>info.id</code></td>
      <td><code>u64</code></td>
      <td>Unique numeric identifier (duplicates rejected at validation)</td>
    </tr>
    <tr>
      <td><code>info.name</code></td>
      <td><code>[]const u8</code></td>
      <td>Human-readable module name</td>
    </tr>
    <tr>
      <td><code>info.capabilities</code></td>
      <td><code>[]const Capability</code></td>
      <td>Capabilities this module provides</td>
    </tr>
    <tr>
      <td><code>context</code></td>
      <td><code>*anyopaque</code></td>
      <td>Opaque pointer to module state</td>
    </tr>
    <tr>
      <td><code>hooks.start_fn</code></td>
      <td>optional</td>
      <td>Called when the runtime starts</td>
    </tr>
    <tr>
      <td><code>hooks.stop_fn</code></td>
      <td>optional</td>
      <td>Called when the runtime stops (reverse registration order)</td>
    </tr>
    <tr>
      <td><code>hooks.command_fn</code></td>
      <td>optional</td>
      <td>Called when a command is dispatched to modules</td>
    </tr>
  </tbody>
</table>

## Registry methods

<table>
  <thead>
    <tr>
      <th>Method</th>
      <th>Description</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>validate()</code></td>
      <td>Check for duplicate module IDs</td>
    </tr>
    <tr>
      <td><code>startAll(runtime)</code></td>
      <td>Call <code>start_fn</code> on all modules</td>
    </tr>
    <tr>
      <td><code>stopAll(runtime)</code></td>
      <td>Call <code>stop_fn</code> on all modules (reverse order)</td>
    </tr>
    <tr>
      <td><code>dispatchCommand(runtime, command)</code></td>
      <td>Call <code>command_fn</code> on all modules</td>
    </tr>
    <tr>
      <td><code>hasCapability(kind)</code></td>
      <td>Check if any module provides a capability</td>
    </tr>
  </tbody>
</table>

## Capability kinds

<table>
  <thead>
    <tr>
      <th>Kind</th>
      <th>Description</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>native_module</code></td>
      <td>A native Zig module</td>
    </tr>
    <tr>
      <td><code>webview</code></td>
      <td>WebView rendering</td>
    </tr>
    <tr>
      <td><code>js_bridge</code></td>
      <td>JavaScript bridge</td>
    </tr>
    <tr>
      <td><code>filesystem</code></td>
      <td>File system access</td>
    </tr>
    <tr>
      <td><code>network</code></td>
      <td>Network access</td>
    </tr>
    <tr>
      <td><code>clipboard</code></td>
      <td>Clipboard access</td>
    </tr>
    <tr>
      <td><code>custom</code></td>
      <td>Custom capability (with a <code>name</code> field)</td>
    </tr>
  </tbody>
</table>

These map to the `capabilities` field in [app.zon](/app-zon). The runtime can query whether any module provides a given capability using `registry.hasCapability(.filesystem)`.

## Registering a plugin in your app

Every bundled plugin exposes a `create(allocator)` function that returns a `Module` value ready to drop into a `ModuleRegistry`. The pattern is the same whether the plugin lives in your `src/` tree or in a third-party package.

```zig
const std = @import("std");
const zero_native = @import("zero_native");
const plugin_clipboard = @import("zero_native/extensions/plugin_clipboard.zig");

pub fn main() !void {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();

    // 1. Build the module with the plugin's factory.
    const clipboard_module = try plugin_clipboard.create(allocator);

    // 2. Group it (and any others) into a registry.
    const modules = [_]zero_native.extensions.Module{clipboard_module};
    const registry = zero_native.extensions.ModuleRegistry{ .modules = &modules };
    try registry.validate();

    // 3. Hand the registry to the runtime.
    var runtime = zero_native.Runtime.init(.{
        .platform = my_platform,
        .extensions = registry,
    });
    defer runtime.deinit();

    try registry.startAll(runtime.context());
    defer registry.stopAll(runtime.context());

    try runtime.run();
}
```

The `info` block on the module declares the plugin's identity and capabilities. Override it before registration if you need a different id, name, or a custom capability entry:

```zig
var clipboard_module = try plugin_clipboard.create(allocator);
clipboard_module.info.id = 42;
clipboard_module.info.name = "my-clipboard";
clipboard_module.info.capabilities = &.{
    .{ .kind = .clipboard, .name = "text" },
};
```

### How the bridge dispatches a command

When JavaScript calls `window.zero.invoke('clipboard.write_text', payload)`:

1. The bridge looks up the command in the `CommandRegistry` declared in `app.zon`.
2. It validates the call against the active window's [capabilities](/capabilities) (origin, scope, and policy).
3. The bridge calls `registry.dispatchCommand(runtime, command)`, which fans the command out to every module whose `command_fn` is set.
4. Plugins that own the command (matched by `info.id` or by name) handle it; others ignore it.

### End-to-end: clipboard.write_text

Install the plugin in your `main.zig` (snippet above) and declare its command in `app.zon`:

```text
.{
    .name = "my-app",
    .security = .{
        .capabilities = .{
            .clipboard = .{ .enabled = true },
        },
    },
    .bridge = .{
        .commands = .{
            .{ .name = "clipboard.write_text", .module = "clipboard" },
            .{ .name = "clipboard.read_text",  .module = "clipboard" },
        },
    },
}
```

Then call it from the frontend:

```javascript
import { invoke } from "zero-native";

await invoke("clipboard.write_text", "hello from the webview");
const value = await invoke("clipboard.read_text", "");
console.log(value);
```

See [Plugin: Clipboard](/extensions/plugins/clipboard) for the full lifecycle and [Bundled Plugins](/extensions/plugins) for the full list of available plugins.

## Third-party plugins

Out-of-tree plugins use the same `Module` shape as bundled plugins. Register them by providing a `Plugin` table to `Runtime.loadPlugins` or `registry.loadFromManifestWithOptions`:

```zig
const zero_native = @import("zero-native");

const my_plugin = @import("my-plugin");

const custom_plugins = &[_]zero_native.extensions.Plugin{.{
    .name = "my-plugin",
    .create_fn = my_plugin.create,
}};

try runtime.loadPlugins(allocator, io, metadata.plugins, .{
    .custom_plugins = custom_plugins,
    .config = .{
        .app_name = metadata.id,
        .current_version = metadata.version,
        .manifest_url = metadata.updates.feed_url,
        .public_key_b64 = metadata.updates.public_key,
        .check_on_start = metadata.updates.check_on_start,
    },
});
```

The loader checks built-in plugin names first, then falls back to the custom table. If a name is not found in either, `error.UnknownPlugin` is returned. Once loaded, third-party plugins participate in command dispatch and lifecycle hooks exactly like bundled plugins.

## Native JS engine (experimental)

The `js` module provides an abstraction layer for calling into a native JavaScript engine from Zig. This is separate from the WebView bridge and is intended for future native module integrations.

<table>
  <thead>
    <tr>
      <th>Type</th>
      <th>Description</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>Value</code></td>
      <td>Tagged union: <code>null</code>, <code>boolean</code>, <code>number</code>, <code>string</code></td>
    </tr>
    <tr>
      <td><code>Call</code></td>
      <td>A function call: <code>module</code>, <code>function</code>, <code>args</code></td>
    </tr>
    <tr>
      <td><code>Bridge</code></td>
      <td>Validates and dispatches calls via <code>RuntimeHooks</code></td>
    </tr>
    <tr>
      <td><code>NullEngine</code></td>
      <td>Stub that returns <code>EngineUnavailable</code></td>
    </tr>
  </tbody>
</table>
