...
pub const RunOptions = struct {
    ...
    /// Plugin names from the parsed `app.zon`. Forwarded to
    /// `Runtime.loadPlugins` so the example wires `Metadata.plugins` into
    /// the runtime registry. Empty slice skips plugin loading.
