...
pub fn stop(ctx: *anyopaque, _: extensions.RuntimeContext) anyerror!void {
    ...
    for (s.items.items) |item| s.allocator.free(item.label);
...
pub fn command(ctx: *anyopaque, runtime: extensions.RuntimeContext, cmd: extensions.Command) anyerror!void {
    ...
    if (std.mem.eql(u8, cmd.name, "tray.init")) {
        ...
        if (svc) |services| _ = services.createTray(.{ .icon_path = cmd.payload[0..sep], .tooltip = cmd.payload[sep+1..], .items = s.items.items }) catch {};
        ...
        if (svc) |services| _ = services.updateTrayMenu(s.items.items) catch {};
    }
}
...
test "tray add and list" {
    const m = try create(std.testing.allocator);
    defer m.hooks.stop_fn.?(m.context, .{.platform_name="null"}) catch {};
    try m.hooks.command_fn.?(m.context, .{.platform_name="null"}, .{.name="tray.add", .payload="Prefs", .target=ModuleId});
    try m.hooks.command_fn.?(m.context, .{.platform_name="null"}, .{.name="tray.add", .payload="Quit", .target=ModuleId});
    const s: *TrayState = @ptrCast(@alignCast(m.context));
    try std.testing.expectEqual(@as(usize, 2), s.items.items.len);
    try std.testing.expectEqualStrings("Prefs", s.items.items[0].label);
    try std.testing.expectEqualStrings("Quit", s.items.items[1].label);
}
