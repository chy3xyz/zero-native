...
const app_exe_name = "with-plugins";
...
pub fn build(b: *std.Build) void {
    ...
    // Read the bundled webview source at build time so `src/main.zig`
    // can serve it as a `WebViewSource.html`. Keeps the example's HTML
    // in `index.html` at the package root (where readers expect it)
    // while still letting the runtime embed it without depending on
    // the file at runtime.
    var index_file = b.build_root.handle.openFile(b.path("index.html"), .{}) catch |err| {
        std.debug.panic("failed to open index.html: {s}", .{@errorName(err)});
    };
    defer index_file.close();
    var read_buffer: [64 * 1024]u8 = undefined;
    const index_len = index_file.readAll(&read_buffer) catch |err| {
        std.debug.panic("failed to read index.html: {s}", .{@errorName(err)});
    };
    const index_html = read_buffer[0..index_len];
